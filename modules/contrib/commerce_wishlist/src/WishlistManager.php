...
class WishlistManager implements WishlistManagerInterface {
  ...
  public function merge(WishlistInterface $source, WishlistInterface $target, $save = TRUE) {
    foreach ($source->getItems() as $wishlist_item) {
      ...
      // Add wishlist item directly, because each item could have own comment.
      // With wishlist manager we can combine items, but comment is going
      // to be lost then.
      // Use duplication of items, because on deleting $wishlist_merge
      // which is merged, all their wishlist items are going to be deleted.
    }

    // Save wishlist if there is a flag. Also delete old one then.
    if ($save) {
      $target->save();
      // TODO: what with old url/code if user already shared wishlist.
      $source->delete();
    }
